...
// Event creation state management - INITIALLY ALL DISABLED
...
// Initialize state on mount - ENHANCED FOR EDIT MODE
onMounted(async () => {
  // Check for event ID in route params (for editing from event list)
  const routeEventId = route.params.id
  
  // Check for existing event ID in localStorage
  const savedEventId = localStorage.getItem('currentEventId')
  const editEventId = localStorage.getItem('editEventId')
  
  // Priority: Route param > Edit event ID > Current event ID
  const targetEventId = routeEventId || editEventId || savedEventId
  
  if (targetEventId) {
    try {
      eventId.value = targetEventId
      isEditMode.value = true
      
      console.log('🔍 Loading event for editing:', targetEventId)
      const data = await getEventDetails(targetEventId)
      ...
      if (data && data.data) {
        ...
        console.log('📋 Event loaded for editing:', {
          id: eventId.value,
          name: data.data.name,
          source: routeEventId ? 'route' : editEventId ? 'edit' : 'current'
        })
        
        // Store as current event for persistence
        localStorage.setItem('currentEventId', targetEventId)
        localStorage.setItem('currentEventName', data.data.name || 'Unnamed Event')
        
        // Clear edit event ID if it was used
        if (editEventId) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Load Failed',
        detail: 'Failed to load event data. Starting fresh.',
        life: 3000
      })
    }
  } else {
    // No event ID found - this is a new event creation
    isEditMode.value = false
    console.log('🆕 Starting new event creation')
  }

  // Check for tab query parameter (from manage tickets)
  if (route.query.tab === 'tickets' && isBasicInfoCompleted.value) {
...
// Clear event data when leaving the page (only for new events)
onBeforeUnmount(() => {
  // Only clear if this was a new event creation, not editing
  if (!isEditMode.value) {
    console.log('🚪 Leaving Create Event page - clearing current event ID')
    localStorage.removeItem('currentEventId')
    localStorage.removeItem('currentEventName')
  } else {
    console.log('🚪 Leaving Edit Event page - keeping event data')
  }
})
