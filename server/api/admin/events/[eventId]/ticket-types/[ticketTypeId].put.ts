export default defineEventHandler(async (event) => {
  try {
    ...
    console.error('Error in ticket type update proxy:', error)
    
    // Handle different error types
    if (error.status || error.statusCode) {
      throw createError({
        statusCode: error.status || error.statusCode,
        statusMessage: error.message || error.statusMessage || 'Ticket type update failed'
      })
    }
    ...
    throw createError({
      statusCode: 500,
      statusMessage: 'Internal server error during ticket type update'
    })
  }
})
