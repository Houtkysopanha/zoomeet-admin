  ...
  />
    ...
    <div class="">
      <div class="flex items-center justify-between mb-6">
        ...
        <div>
            <span class="text-blue-600 font-medium px-3 py-1 rounded-full bg-blue-50">Now 3 event available</span>
        </div>
        </div>
      ...
      <!-- Event Cards - Grid Layout (Original Design) -->
    ...
    <div class="font-medium text-gray-800">{{ event.time }}</div>
      ...
      <div v-else class="space-y-4">
        <div 
          v-for="event in filteredEvents" 
          :key="event.id"
          class="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200 cursor-pointer bg-white flex"
          @click="selectEvent(event)"
        >
...
const selectEvent = (event) => {
  ...
  // Removed toast.success to avoid issues
}
</script>
