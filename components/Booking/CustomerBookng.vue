  ...
  <div class="bg-gray-50">
    ...
    <div class="w-full">
      ...
      <div>
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <span class="text-blue-600 font-medium px-3 py-1 rounded-full bg-blue-50">
                <span v-if="isLoading">Loading events...</span>
                <span v-else>Now {{ events.length }} event{{ events.length !== 1 ? 's' : '' }} available</span>
              </span>
            </div>
          </div>
        ...
        <div v-else-if="events.length === 0" class="text-center py-12">
          ...
          <p class="text-gray-600">There are currently no events available for booking.</p>
        </div>

        <!-- Event Cards - Grid Layout -->
        <div v-else-if="layoutView === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
          ...
          >
            ...
            <div class="p-4 flex-1 flex flex-col">
              <h3 class="font-semibold text-gray-800 mb-3 text-sm leading-tight line-clamp-2 flex-grow">
                {{ event.title }}
              </h3>
              <div class="flex items-start text-gray-600 mb-4 min-h-[20px]">
                ...
                <span class="text-sm line-clamp-1">{{ event.location }}</span>
                 ...
                 <div class="flex flex-col sm:flex-row justify-around items-center bg-white border border-white p-3 rounded-xl shadow-md">
      <div class="flex flex-col items-center text-center mb-2 sm:mb-0">
        <p class="text-gray-500 text-xs font-medium">Date</p>
        <p class="text-gray-800 text-base md:text-sm font-semibold">{{ event.date}}</p>
      </div>
      ...
      <div class="flex flex-col items-center text-center">
        ...
        <p class="text-gray-800 text-base md:text-sm font-semibold">{{ event.time }}</p>
      </div>
    </div>
        ...
        <div v-else-if="layoutView === 'column'" class="space-y-4">
          <div
            ...
            @click="selectEvent(event)"
          >
            <div class="w-48 h-36 sm:w-52 sm:h-40 md:w-48 md:h-36 relative overflow-hidden flex-shrink-0">
              <img :src="event.image" :alt="event.title" class="w-full h-full object-cover rounded-l-2xl" />
            </div>
            <div class="flex-1 p-4 flex flex-col justify-between min-h-[144px]">
              ...
              <div class="flex gap-8 text-sm mt-auto">
                <div>
                  ...
                  <div class="font-medium text-gray-800 text-xs">{{ event.date }}</div>
                </div>
                <div>
                  <div class="text-gray-500 mb-1">Time</div>
                  <div class="font-medium text-gray-800 text-xs">{{ event.time }}</div>
                </div>
              </div>
            </div>
          </div>
...
const getEventImage = (event) => {
  return event.cover_image_url || event.card_background_url || event.event_background_url || img1
}
...
const loadEvents = async () => {
  ...
  try {
    ...
    errorMessage.value = error.message || 'Failed to load events. Please try again.'
